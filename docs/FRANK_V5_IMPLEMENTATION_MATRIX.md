...
| --- | --- | --- | --- | --- | --- |
| R-001 | Provider factory assumptions keep runtime pinned to `providers.openai`. | V5-002, V5-004, V5-016 | High | Add named provider construction tests and keep legacy provider tests. Fail closed on unknown provider refs. | Mitigated by named provider runtime tests and V5-016 checked-in fixtures for legacy, local, Ollama, and OpenRouter providers. |
| R-002 | API keys or authorization headers leak through CLI, status, logs, health errors, or tests. | V5-002, V5-006, V5-007, V5-014, V5-016 | Critical | Use redacted config views, error classes, fake secrets in tests, and no raw request/response body logging. | Mitigated for V5 route, health, policy-denial, and fake-provider test paths; broader provider error logging remains outside the V5 matrix. |
| R-003 | Tool schemas leak to weak local models. | V5-003, V5-010, V5-012, V5-016 | Critical | Gate tool definitions immediately before provider calls. Add fake provider tests that assert zero schemas. | Mitigated by V5-003/V5-012 gates and V5-016 local fixture fake-provider assertion for zero tools. |
| R-004 | Local/cloud fallback violates policy. | V5-008, V5-010, V5-016 | Critical | Enforce resolver fallback rules in runtime and test local-to-cloud denial by default. | Mitigated for V5 by preflight runtime tests and V5-016 mixed local/cloud fallback denied and allowed fixtures. |
| R-005 | Legacy config or raw `-M` override breaks. | V5-002, V5-004, V5-016, V5-017 | High | Keep implicit legacy profile and raw model compatibility tests in every runtime slice touching startup. | Mitigated by V5-016 legacy fixture coverage and V5-017 migration docs. |
| R-006 | CLI model override ambiguity between alias, model ref, and raw provider model changes behavior. | V5-002, V5-010, V5-016 | Medium | Preserve V5-001 rule: model ref wins, alias resolves deterministically, unknown value becomes raw legacy provider model only on legacy-compatible path. Document this clearly. | Mitigated by alias, model-ref, raw legacy, policy-denial tests, and V5-017 migration docs. |
| R-007 | Mission step policy conflicts with global routing or job policy. | V5-009, V5-010, V5-012 | High | Implement narrowing semantics with tests. Fail closed on contradictions instead of widening policy. | Mitigated for V5 by V5-009 schema validation, V5-010 routing, V5-012 step authority checks, and V5-016 acceptance coverage. |
| R-008 | Tests accidentally require live llama.cpp, Ollama, OpenAI, OpenRouter, phone, or API keys. | V5-007, V5-014, V5-016, V5-018 | High | Use `httptest`, fake providers, and placeholder credentials only. Mark live probes as manual docs, not automated gates. | Mitigated by V5-016 checked-in fixtures with placeholder secrets, `httptest`-only acceptance coverage, and V5-018 full test pass. |
| R-009 | Android runtime docs drift from actual V5 config and CLI behavior. | V5-015, V5-017 | Medium | Write docs after health and fallback command shapes are implemented. Include config snippets that are covered by config parser tests where practical. | Mitigated by V5-015 phone runtime docs and V5-017 migration guide links. |
| R-010 | Temperature and request override support require provider interface changes that introduce shared mutable state. | V5-004 | Medium | Prefer per-call options or immutable provider clones. Add concurrency-safe tests if shared provider instances remain. | Mitigated by V5-004 request override implementation and V5-016 fake-provider fixture assertions for request fields. |
