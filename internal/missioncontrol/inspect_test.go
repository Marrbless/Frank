...
func TestFormatInspectSummarySurfacesFrankMicrosoftOnboardingPreflight(t *testing.T) {
	t.Parallel()
			...
			Steps: []Step{
				{
					...
					RequiredAuthority: AuthorityTierLow,
					AllowedTools:      []string{"read"},
					SuccessCriteria:   []string{"confirm microsoft onboarding"},
					IdentityMode:      IdentityModeAgentAlias,
					FrankObjectRefs: []FrankRegistryObjectRef{
						{Kind: FrankRegistryObjectKindIdentity, ObjectID: fixtures.identity.IdentityID},
						{Kind: FrankRegistryObjectKindAccount, ObjectID: fixtures.account.AccountID},
	...
	steps := mustJSONArray(t, got["steps"], "inspect.steps")
	step := steps[0].(map[string]any)
	assertResolvedFrankMicrosoftOnboardingPreflightJSONEnvelope(t, step["frank_microsoft_onboarding_preflight"])
	if _, ok := step["campaign_preflight"]; ok {
		t.Fatalf("campaign_preflight = %#v, want omitted on microsoft onboarding path", step["campaign_preflight"])
	}
	if _, ok := step["treasury_preflight"]; ok {
		t.Fatalf("treasury_preflight = %#v, want omitted on microsoft onboarding path", step["treasury_preflight"])
	}
}

func TestFormatInspectSummarySurfacesFrankLinkedInOnboardingPreflight(t *testing.T) {
	t.Parallel()
			...
			Steps: []Step{
				{
					...
					RequiredAuthority: AuthorityTierLow,
					AllowedTools:      []string{"read"},
					SuccessCriteria:   []string{"confirm linkedin onboarding"},
					IdentityMode:      IdentityModeAgentAlias,
					FrankObjectRefs: []FrankRegistryObjectRef{
						{Kind: FrankRegistryObjectKindIdentity, ObjectID: fixtures.identity.IdentityID},
						{Kind: FrankRegistryObjectKindAccount, ObjectID: fixtures.account.AccountID},
	...
	steps := mustJSONArray(t, got["steps"], "inspect.steps")
	step := steps[0].(map[string]any)
	assertResolvedFrankLinkedInOnboardingPreflightJSONEnvelope(t, step["frank_linkedin_onboarding_preflight"])
	if _, ok := step["campaign_preflight"]; ok {
		t.Fatalf("campaign_preflight = %#v, want omitted on linkedin onboarding path", step["campaign_preflight"])
	}
	if _, ok := step["treasury_preflight"]; ok {
		t.Fatalf("treasury_preflight = %#v, want omitted on linkedin onboarding path", step["treasury_preflight"])
	}
}
