...
func TestPersistProjectedRuntimeStateProjectsFrankZohoBounceEvidenceAppendOnly(t *testing.T) {
	t.Parallel()
	...
	records, err := ListCommittedFrankZohoBounceEvidenceRecords(root, job.ID)
	if err != nil {
		t.Fatalf("ListCommittedFrankZohoBounceEvidenceRecords() error = %v", err)
	}
	if len(records) != 2 {
		t.Fatalf("ListCommittedFrankZohoBounceEvidenceRecords() len = %d, want 2", len(records))
	}
	if records[0].OriginalMIMEMessageID != evidenceOne.OriginalMIMEMessageID {
		t.Fatalf("records[0].OriginalMIMEMessageID = %q, want %q", records[0].OriginalMIMEMessageID, evidenceOne.OriginalMIMEMessageID)
	}
	if records[0].OutboundActionID != evidenceOne.OutboundActionID {
		t.Fatalf("records[0].OutboundActionID = %q, want %q", records[0].OutboundActionID, evidenceOne.OutboundActionID)
	}
	if records[1].ProviderMessageID != evidenceTwo.ProviderMessageID {
		t.Fatalf("records[1].ProviderMessageID = %q, want %q", records[1].ProviderMessageID, evidenceTwo.ProviderMessageID)
	}
	if records[1].CampaignID != "" || records[1].OutboundActionID != "" {
		t.Fatalf("records[1] attribution = (%q, %q), want unattributed evidence", records[1].CampaignID, records[1].OutboundActionID)
	}
}
