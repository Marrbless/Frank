...
func LoadCommittedCampaignZohoEmailSendGateDecision(root string, campaign CampaignRecord) (CampaignZohoEmailSendGateDecision, error) {
	...
	records, err := ListCommittedCampaignZohoEmailOutboundActionRecordsByCampaign(root, normalized.CampaignID)
	if err != nil {
		return CampaignZohoEmailSendGateDecision{}, err
	}
	return DeriveCampaignZohoEmailSendGateDecision(normalized, records)
}

func DeriveCampaignZohoEmailSendGateDecision(campaign CampaignRecord, records []CampaignZohoEmailOutboundActionRecord) (CampaignZohoEmailSendGateDecision, error) {
	...
	for _, stopCondition := range normalizedCampaign.StopConditions {
		limit, ok := campaignZohoEmailVerifiedSendStopLimit(stopCondition)
		if !ok {
			return CampaignZohoEmailSendGateDecision{}, fmt.Errorf("campaign zoho email stop_condition %q is not evaluable from committed outbound action records", stopCondition)
		}
