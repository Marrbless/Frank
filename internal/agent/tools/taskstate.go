...
func (s *TaskState) applyDiscordOwnerControlOnboardingForStep(job missioncontrol.Job, stepID string, now time.Time) error {
	if s == nil {
		return nil
	}
	...
	s.mu.Lock()
	root := strings.TrimSpace(s.missionStoreRoot)
	hook := s.discordOwnerControlOnboardingHook
	s.mu.Unlock()
	job.MissionStoreRoot = root
	...
	if ec.Step == nil || !missioncontrol.DeclaresFrankDiscordOwnerControlOnboarding(*ec.Step) {
		return nil
	}
	ec.MissionStoreRoot = root
...
func (s *TaskState) applyGitHubOnboardingForStep(job missioncontrol.Job, stepID string, now time.Time) error {
	if s == nil {
		return nil
	}
	...
	if ec.Step == nil || !missioncontrol.DeclaresFrankGitHubOnboarding(*ec.Step) {
		return nil
	}
	ec.MissionStoreRoot = root
...
func (s *TaskState) applyPayPalOnboardingForStep(job missioncontrol.Job, stepID string, now time.Time) error {
	if s == nil {
		return nil
	}
	...
	if ec.Step == nil || !missioncontrol.DeclaresFrankPayPalOnboarding(*ec.Step) {
		return nil
	}
	ec.MissionStoreRoot = root
...
func (s *TaskState) applyLinkedInOnboardingForStep(job missioncontrol.Job, stepID string, now time.Time) error {
	if s == nil {
		return nil
	}
	...
	if ec.Step == nil || !missioncontrol.DeclaresFrankLinkedInOnboarding(*ec.Step) {
		return nil
	}
	ec.MissionStoreRoot = root

	bundle, ok, err := missioncontrol.ResolveExecutionContextFrankLinkedInOnboardingBundle(ec)
	if err != nil {
		return err
	}
	if !ok || hook == nil {
