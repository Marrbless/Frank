...
func TestFrankZohoCampaignSendStabilizesTimestampOrderingUnderClockRegression(t *testing.T) {
	originalVerify := verifyFrankZohoCampaignSendProof
	t.Cleanup(func() {
		verifyFrankZohoCampaignSendProof = originalVerify
	})

	prepareAt := time.Date(2026, 4, 15, 21, 45, 2, 0, time.UTC)
	sendAt := prepareAt.Add(-time.Second)
	verifyAt := prepareAt.Add(-2 * time.Second)
	useTaskStateNowSequence(t, prepareAt, sendAt, verifyAt)
	...
	job := missioncontrol.Job{
		ID:           "job-frank-zoho-send-ordering",
		MaxAuthority: missioncontrol.AuthorityTierHigh,
		AllowedTools: []string{tool.Name()},
		Plan: missioncontrol.Plan{
			ID: "plan-frank-zoho-send-ordering",
			Steps: []missioncontrol.Step{
				{
					ID:                "send-outbound-email",
					Type:              missioncontrol.StepTypeDiscussion,
	...
	if records[0].SentAt.Before(records[0].PreparedAt) {
		t.Fatalf("SentAt = %s, PreparedAt = %s, want sent_at on or after prepared_at", records[0].SentAt.Format(time.RFC3339Nano), records[0].PreparedAt.Format(time.RFC3339Nano))
	}
	if records[0].VerifiedAt.Before(records[0].SentAt) {
		t.Fatalf("VerifiedAt = %s, SentAt = %s, want verified_at on or after sent_at", records[0].VerifiedAt.Format(time.RFC3339Nano), records[0].SentAt.Format(time.RFC3339Nano))
	}
}
...
func TestPrepareFrankZohoCampaignFollowUpPersistsLinkedAction(t *testing.T) {
	...
	parent.MIMEMessageID = "<parent-follow-up@example.test>"
	...
	reply := missioncontrol.FrankZohoInboundReply{
		...
		ProviderAccountID:  "3323462000000008002",
		ProviderMessageID:  "1711540357880102000",
		ProviderMailID:     "<reply-follow-up-1@zoho.test>",
		MIMEMessageID:      "<reply-follow-up-1@example.test>",
		InReplyTo:          parent.MIMEMessageID,
		References:         []string{"<seed@example.test>", parent.MIMEMessageID},
		FromAddress:        "person@example.com",
		FromDisplayName:    "Person One",
		FromAddressCount:   1,
		Subject:            "Re: Frank intro",
	...
	replyRecord := persistFrankZohoCampaignRuntime(t, root, "job-frank-zoho-follow-up-history", campaign.CampaignID, []missioncontrol.CampaignZohoEmailOutboundAction{parent}, []missioncontrol.FrankZohoInboundReply{reply}, now)[0]
	...
	job := missioncontrol.Job{
		ID:           "job-frank-zoho-follow-up-prepare",
		MaxAuthority: missioncontrol.AuthorityTierHigh,
		AllowedTools: []string{FrankZohoSendEmailToolName},
		Plan: missioncontrol.Plan{
			ID: "plan-frank-zoho-follow-up-prepare",
			Steps: []missioncontrol.Step{
				{
					ID:                "send-outbound-email",
					Type:              missioncontrol.StepTypeDiscussion,
	...
	if _, skip, err := state.PrepareFrankZohoCampaignSend(args); err != nil {
		...
		t.Fatal("PrepareFrankZohoCampaignSend(follow-up) skip = true, want new prepared follow-up")
	}
	...
	record := records[0]
	if record.ReplyToInboundReplyID != replyRecord.ReplyID {
		t.Fatalf("ReplyToInboundReplyID = %q, want %q", record.ReplyToInboundReplyID, replyRecord.ReplyID)
	}
	if record.ReplyToOutboundActionID != parent.ActionID {
		t.Fatalf("ReplyToOutboundActionID = %q, want %q", record.ReplyToOutboundActionID, parent.ActionID)
	}
	if len(record.Addressing.To) != 1 || record.Addressing.To[0] != "person@example.com" {
		t.Fatalf("Addressing.To = %#v, want uniquely resolved reply sender", record.Addressing.To)
	}
	if len(record.Addressing.CC) != 0 || len(record.Addressing.BCC) != 0 {
		t.Fatalf("Addressing = %#v, want reply follow-up to target only the committed sender", record.Addressing)
	}
	...
	if workItems[0].State != string(missioncontrol.CampaignZohoEmailReplyWorkItemStateClaimed) {
		t.Fatalf("workItems[0].State = %q, want claimed", workItems[0].State)
	}
	if workItems[0].ClaimedFollowUpActionID != record.ActionID {
		t.Fatalf("workItems[0].ClaimedFollowUpActionID = %q, want %q", workItems[0].ClaimedFollowUpActionID, record.ActionID)
	}
}
...
func TestPrepareFrankZohoCampaignSendAutoSelectsOldestReplyWorkItem(t *testing.T) {
	...
	parent := mustBuildVerifiedFrankZohoCampaignAction(t, "send-outbound-email", campaign.CampaignID, "Frank intro", now)
	parent.MIMEMessageID = "<parent-autoselect@example.test>"
	if err := missioncontrol.ValidateCampaignZohoEmailOutboundAction(parent); err != nil {
		t.Fatalf("ValidateCampaignZohoEmailOutboundAction(parent) error = %v", err)
	}
	olderReply := missioncontrol.FrankZohoInboundReply{
		StepID:             "sync-replies",
		Provider:           "zoho_mail",
		ProviderAccountID:  "3323462000000008002",
		ProviderMessageID:  "1711540357880103000",
		ProviderMailID:     "<reply-autoselect-1@zoho.test>",
		MIMEMessageID:      "<reply-autoselect-1@example.test>",
		InReplyTo:          parent.MIMEMessageID,
		FromAddress:        "older@example.com",
		FromDisplayName:    "Older Person",
		FromAddressCount:   1,
		Subject:            "Re: Frank intro",
		ReceivedAt:         now.Add(time.Minute),
		OriginalMessageURL: "https://mail.zoho.test/api/accounts/3323462000000008002/messages/1711540357880103000/originalmessage",
	}
	newerReply := missioncontrol.FrankZohoInboundReply{
		StepID:             "sync-replies",
		Provider:           "zoho_mail",
		ProviderAccountID:  "3323462000000008002",
		ProviderMessageID:  "1711540357880103001",
		ProviderMailID:     "<reply-autoselect-2@zoho.test>",
		MIMEMessageID:      "<reply-autoselect-2@example.test>",
		InReplyTo:          parent.MIMEMessageID,
		FromAddress:        "newer@example.com",
		FromDisplayName:    "Newer Person",
		FromAddressCount:   1,
		Subject:            "Re: Frank intro",
		ReceivedAt:         now.Add(2 * time.Minute),
		OriginalMessageURL: "https://mail.zoho.test/api/accounts/3323462000000008002/messages/1711540357880103001/originalmessage",
	}
	replyRecords := persistFrankZohoCampaignRuntime(t, root, "job-frank-zoho-autoselect-history", campaign.CampaignID, []missioncontrol.CampaignZohoEmailOutboundAction{parent}, []missioncontrol.FrankZohoInboundReply{olderReply, newerReply}, now)
	...
	job := missioncontrol.Job{
		ID:           "job-frank-zoho-autoselect-prepare",
		MaxAuthority: missioncontrol.AuthorityTierHigh,
		AllowedTools: []string{FrankZohoSendEmailToolName},
		Plan: missioncontrol.Plan{
			ID: "plan-frank-zoho-autoselect-prepare",
			Steps: []missioncontrol.Step{
				{
					ID:                "send-outbound-email",
					Type:              missioncontrol.StepTypeDiscussion,
	...
	if appended, err := state.SyncFrankZohoCampaignInboundReplies(); err != nil {
		t.Fatalf("SyncFrankZohoCampaignInboundReplies() error = %v", err)
	} else if appended != 0 {
		t.Fatalf("SyncFrankZohoCampaignInboundReplies() appended = %d, want 0 new mailbox replies", appended)
	}

	if _, skip, err := state.PrepareFrankZohoCampaignSend(map[string]interface{}{
		"subject": "Re: Frank intro",
		"body":    "Thanks for writing back.",
	}); err != nil {
		t.Fatalf("PrepareFrankZohoCampaignSend(auto-select) error = %v", err)
	} else if skip {
		t.Fatal("PrepareFrankZohoCampaignSend(auto-select) skip = true, want claimed follow-up preparation")
	}
	...
	if records[0].ReplyToInboundReplyID != replyRecords[0].ReplyID {
		t.Fatalf("ReplyToInboundReplyID = %q, want oldest reply %q", records[0].ReplyToInboundReplyID, replyRecords[0].ReplyID)
	}
}
...
func TestPrepareFrankZohoCampaignFollowUpVerifiedReplayRespondsWorkItem(t *testing.T) {
	...
	parent := mustBuildVerifiedFrankZohoCampaignAction(t, "send-outbound-email", campaign.CampaignID, "Frank intro", now)
	parent.MIMEMessageID = "<parent-responded@example.test>"
	if err := missioncontrol.ValidateCampaignZohoEmailOutboundAction(parent); err != nil {
		t.Fatalf("ValidateCampaignZohoEmailOutboundAction(parent) error = %v", err)
	}
	reply := missioncontrol.FrankZohoInboundReply{
		...
		ProviderMailID:     "<reply-responded-1@zoho.test>",
		MIMEMessageID:      "<reply-responded-1@example.test>",
		InReplyTo:          parent.MIMEMessageID,
		FromAddress:        "person@example.com",
		FromDisplayName:    "Person One",
		FromAddressCount:   1,
	...
	replyRecord := persistFrankZohoCampaignRuntime(t, root, "job-frank-zoho-responded-history", campaign.CampaignID, []missioncontrol.CampaignZohoEmailOutboundAction{parent}, []missioncontrol.FrankZohoInboundReply{reply}, now)[0]
	preparedFollowUp, err := missioncontrol.BuildCampaignZohoEmailOutboundPreparedReplyAction(
		"send-outbound-email",
		campaign.CampaignID,
		"3323462000000008002",
		"frank@omou.online",
		"Frank",
		missioncontrol.CampaignZohoEmailAddressing{To: []string{"person@example.com"}},
		"Re: Frank intro",
		"plaintext",
		"Hello from Frank",
	...
	followUp, err := missioncontrol.BuildCampaignZohoEmailOutboundSentAction(preparedFollowUp, missioncontrol.FrankZohoSendReceipt{
		Provider:           "zoho_mail",
		ProviderAccountID:  "3323462000000008002",
		FromAddress:        "frank@omou.online",
		FromDisplayName:    "Frank",
		ProviderMessageID:  "1711540357880105000",
		ProviderMailID:     "<mail-responded-1@zoho.test>",
		MIMEMessageID:      "<mail-responded-1@example.test>",
		OriginalMessageURL: "https://mail.zoho.test/api/accounts/3323462000000008002/messages/1711540357880105000/originalmessage",
	}, now.Add(2*time.Minute).Add(10*time.Second))
	if err != nil {
		t.Fatalf("BuildCampaignZohoEmailOutboundSentAction(followUp) error = %v", err)
	}
	followUp.State = missioncontrol.CampaignZohoEmailOutboundActionStateVerified
	followUp.VerifiedAt = now.Add(2 * time.Minute).Add(20 * time.Second)
	if err := missioncontrol.ValidateCampaignZohoEmailOutboundAction(followUp); err != nil {
		t.Fatalf("ValidateCampaignZohoEmailOutboundAction(followUp) error = %v", err)
	}
	claimedItem, err := missioncontrol.BuildCampaignZohoEmailReplyWorkItemOpen(campaign.CampaignID, replyRecord.ReplyID, now.Add(90*time.Second))
	if err != nil {
		t.Fatalf("BuildCampaignZohoEmailReplyWorkItemOpen() error = %v", err)
	}
	claimedItem, err = missioncontrol.BuildCampaignZohoEmailReplyWorkItemClaimed(claimedItem, followUp.ActionID, now.Add(2*time.Minute))
	if err != nil {
	...
	job := missioncontrol.Job{
		ID:           "job-frank-zoho-responded-prepare",
		MaxAuthority: missioncontrol.AuthorityTierHigh,
		AllowedTools: []string{FrankZohoSendEmailToolName},
		Plan: missioncontrol.Plan{
			ID: "plan-frank-zoho-responded-prepare",
			Steps: []missioncontrol.Step{
				{
					ID:                "send-outbound-email",
					Type:              missioncontrol.StepTypeDiscussion,
	...
	ec.Runtime = &missioncontrol.JobRuntimeState{
		JobID:                            job.ID,
		State:                            missioncontrol.JobStateRunning,
		ActiveStepID:                     job.Plan.Steps[0].ID,
		CampaignZohoEmailOutboundActions: []missioncontrol.CampaignZohoEmailOutboundAction{followUp},
		CampaignZohoEmailReplyWorkItems:  []missioncontrol.CampaignZohoEmailReplyWorkItem{claimedItem},
		FrankZohoInboundReplies:          []missioncontrol.FrankZohoInboundReply{replyRecord},
		CreatedAt:                        now,
		UpdatedAt:                        now,
		StartedAt:                        now,
		ActiveStepAt:                     now,
	...
	if _, skip, err := state.PrepareFrankZohoCampaignSend(map[string]interface{}{
		...
		t.Fatalf("PrepareFrankZohoCampaignSend(verified replay) error = %v", err)
	} else if !skip {
		t.Fatal("PrepareFrankZohoCampaignSend(verified replay) skip = false, want replay short-circuit")
	}
	...
	if workItems[0].State != string(missioncontrol.CampaignZohoEmailReplyWorkItemStateResponded) {
		t.Fatalf("workItems[0].State = %q, want responded", workItems[0].State)
	}
}
...
func TestPrepareFrankZohoCampaignFollowUpBlocksDuplicateUnresolvedAction(t *testing.T) {
	...
	root, _, container := writeTaskStateTreasuryFixtures(t)
	campaign := mustStoreFrankZohoAddressedCampaignFixture(t, root, container)
	now := time.Date(2026, 4, 15, 21, 30, 0, 0, time.UTC)

	parent := mustBuildVerifiedFrankZohoCampaignAction(t, "send-outbound-email", campaign.CampaignID, "Frank intro", now)
	parent.MIMEMessageID = "<parent-duplicate@example.test>"
	if err := missioncontrol.ValidateCampaignZohoEmailOutboundAction(parent); err != nil {
		t.Fatalf("ValidateCampaignZohoEmailOutboundAction(parent) error = %v", err)
	}
	reply := missioncontrol.FrankZohoInboundReply{
		...
		ProviderAccountID:  "3323462000000008002",
		ProviderMessageID:  "1711540357880102001",
		ProviderMailID:     "<reply-follow-up-2@zoho.test>",
		MIMEMessageID:      "<reply-follow-up-2@example.test>",
		InReplyTo:          parent.MIMEMessageID,
		FromAddress:        "person@example.com",
		FromDisplayName:    "Person One",
		FromAddressCount:   1,
	...
	existing, err := missioncontrol.BuildCampaignZohoEmailOutboundPreparedReplyAction(
		"send-outbound-email",
		campaign.CampaignID,
		"3323462000000008002",
		"frank@omou.online",
	...
	if err != nil {
		t.Fatalf("BuildCampaignZohoEmailOutboundPreparedReplyAction(existing) error = %v", err)
	}
	persistFrankZohoCampaignRuntime(t, root, "job-frank-zoho-follow-up-existing", campaign.CampaignID, []missioncontrol.CampaignZohoEmailOutboundAction{existing}, nil, now.Add(2*time.Minute))
	...
	job := missioncontrol.Job{
		ID:           "job-frank-zoho-follow-up-duplicate",
		MaxAuthority: missioncontrol.AuthorityTierHigh,
		AllowedTools: []string{FrankZohoSendEmailToolName},
		Plan: missioncontrol.Plan{
			ID: "plan-frank-zoho-follow-up-duplicate",
			Steps: []missioncontrol.Step{
				{
					ID:                "send-outbound-email",
					Type:              missioncontrol.StepTypeDiscussion,
	...
	_, skip, err := state.PrepareFrankZohoCampaignSend(map[string]interface{}{
		"inbound_reply_id": replyRecord.ReplyID,
		"subject":          "Re: Frank intro 2",
		"body":             "Second follow-up",
	})
	if err == nil {
		t.Fatal("PrepareFrankZohoCampaignSend(duplicate) error = nil, want unresolved follow-up block")
	}
	if skip {
		t.Fatal("PrepareFrankZohoCampaignSend(duplicate) skip = true, want hard block before preparation")
	}
	if !strings.Contains(err.Error(), "already has unresolved follow-up action") {
		t.Fatalf("PrepareFrankZohoCampaignSend(duplicate) error = %q, want unresolved follow-up block", err)
	}
}

func TestPrepareFrankZohoCampaignFollowUpDuplicateBlockStabilizesTimestampOrderingUnderClockRegression(t *testing.T) {
	originalRead := readFrankZohoCampaignInboundReplies
	disableFrankZohoCampaignBounceEvidenceRead(t)
	t.Cleanup(func() {
		readFrankZohoCampaignInboundReplies = originalRead
	...
	later := time.Date(2026, 4, 15, 21, 30, 2, 0, time.UTC)
	earlier := later.Add(-time.Second)
	useTaskStateNowSequence(t, later, earlier)
	...
	parent := mustBuildVerifiedFrankZohoCampaignAction(t, "send-outbound-email", campaign.CampaignID, "Frank intro", now)
	parent.MIMEMessageID = "<parent-duplicate-ordering@example.test>"
	if err := missioncontrol.ValidateCampaignZohoEmailOutboundAction(parent); err != nil {
		t.Fatalf("ValidateCampaignZohoEmailOutboundAction(parent) error = %v", err)
	}
	reply := missioncontrol.FrankZohoInboundReply{
		...
		Provider:           "zoho_mail",
		ProviderAccountID:  "3323462000000008002",
		ProviderMessageID:  "1711540357880102999",
		ProviderMailID:     "<reply-follow-up-ordering@zoho.test>",
		MIMEMessageID:      "<reply-follow-up-ordering@example.test>",
		InReplyTo:          parent.MIMEMessageID,
		FromAddress:        "person@example.com",
		FromDisplayName:    "Person One",
		FromAddressCount:   1,
		Subject:            "Re: Frank intro",
		ReceivedAt:         now.Add(time.Minute),
		OriginalMessageURL: "https://mail.zoho.test/api/accounts/3323462000000008002/messages/1711540357880102999/originalmessage",
	}
	replyRecord := persistFrankZohoCampaignRuntime(t, root, "job-frank-zoho-follow-up-history-ordering", campaign.CampaignID, []missioncontrol.CampaignZohoEmailOutboundAction{parent}, []missioncontrol.FrankZohoInboundReply{reply}, now)[0]
	...
	persistFrankZohoCampaignRuntime(t, root, "job-frank-zoho-follow-up-existing-ordering", campaign.CampaignID, []missioncontrol.CampaignZohoEmailOutboundAction{existing}, nil, now.Add(2*time.Minute))
	...
	job := missioncontrol.Job{
		ID:           "job-frank-zoho-follow-up-duplicate-ordering",
		MaxAuthority: missioncontrol.AuthorityTierHigh,
		AllowedTools: []string{FrankZohoSendEmailToolName},
		Plan: missioncontrol.Plan{
			ID: "plan-frank-zoho-follow-up-duplicate-ordering",
			Steps: []missioncontrol.Step{
				{
					ID:                "send-outbound-email",
					Type:              missioncontrol.StepTypeDiscussion,
	...
	for _, item := range workItems {
		if item.InboundReplyID != replyRecord.ReplyID {
			continue
		}
		found = true
		if item.UpdatedAt.Before(item.CreatedAt) {
			t.Fatalf("reply work item UpdatedAt = %s, CreatedAt = %s, want updated_at on or after created_at", item.UpdatedAt.Format(time.RFC3339Nano), item.CreatedAt.Format(time.RFC3339Nano))
		}
	}
	if !found {
		t.Fatalf("reply work item for inbound_reply_id %q not found", replyRecord.ReplyID)
	}
}

func TestFrankZohoSendEmailToolUsesZohoReplyEndpointForCampaignFollowUp(t *testing.T) {
	t.Parallel()
	...
	parent := mustBuildVerifiedFrankZohoCampaignAction(t, "send-outbound-email", campaign.CampaignID, "Frank intro", now)
	parent.MIMEMessageID = "<parent-reply-send@example.test>"
	if err := missioncontrol.ValidateCampaignZohoEmailOutboundAction(parent); err != nil {
		t.Fatalf("ValidateCampaignZohoEmailOutboundAction(parent) error = %v", err)
	}
	reply := missioncontrol.FrankZohoInboundReply{
		...
		ProviderAccountID:  "3323462000000008002",
		ProviderMessageID:  "1711540357880102002",
		ProviderMailID:     "<reply-follow-up-3@zoho.test>",
		MIMEMessageID:      "<reply-follow-up-3@example.test>",
		InReplyTo:          parent.MIMEMessageID,
		FromAddress:        "person@example.com",
		FromDisplayName:    "Person One",
		FromAddressCount:   1,
	...
	replyRecord := persistFrankZohoCampaignRuntime(t, root, "job-frank-zoho-follow-up-send-history", campaign.CampaignID, []missioncontrol.CampaignZohoEmailOutboundAction{parent}, []missioncontrol.FrankZohoInboundReply{reply}, now)[0]

	tool := NewFrankZohoSendEmailTool()
	tool.apiBase = "https://mail.zoho.test/api"
	tool.accessToken = func(context.Context) (string, error) { return "test-zoho-token", nil }
	tool.client = &http.Client{
		Transport: roundTripFunc(func(r *http.Request) (*http.Response, error) {
			gotPath = r.URL.Path
			if err := json.NewDecoder(r.Body).Decode(&gotBody); err != nil {
			...
			return &http.Response{
				...
				Body: io.NopCloser(strings.NewReader(`{
					"status": {"code": 200, "description": "success"},
					"data": {"messageId": 1711540357880103000}
				}`)),
			}, nil
		}),
	}
	...
	if gotPath != "/api/accounts/3323462000000008002/messages/1711540357880102002" {
		t.Fatalf("request path = %q, want provider-threaded reply endpoint", gotPath)
	}
	if gotBody["action"] != "reply" {
		t.Fatalf("action = %#v, want reply", gotBody["action"])
	}
	if gotBody["toAddress"] != "person@example.com" {
		t.Fatalf("toAddress = %#v, want committed reply sender", gotBody["toAddress"])
	}
}
...
func useTaskStateNowSequence(t *testing.T, values ...time.Time) {
	t.Helper()
	if len(values) == 0 {
		t.Fatal("useTaskStateNowSequence() requires at least one timestamp")
	}
	original := taskStateNowUTC
	index := 0
	taskStateNowUTC = func() time.Time {
		if index >= len(values) {
			return values[len(values)-1].UTC()
		}
		value := values[index].UTC()
		index++
		return value
	}
	t.Cleanup(func() {
		taskStateNowUTC = original
	})
}
...
func persistFrankZohoCampaignRuntime(t *testing.T, root, jobID, campaignID string, actions []missioncontrol.CampaignZohoEmailOutboundAction, replies []missioncontrol.FrankZohoInboundReply, now time.Time) []missioncontrol.FrankZohoInboundReply {
	...
	control, err := missioncontrol.BuildRuntimeControlContext(job, "send-outbound-email")
	if err != nil {
		t.Fatalf("BuildRuntimeControlContext(%q) error = %v", jobID, err)
	}
	...
	runtime := missioncontrol.JobRuntimeState{
		...
		State:                            missioncontrol.JobStateRunning,
		ActiveStepID:                     "send-outbound-email",
		CampaignZohoEmailOutboundActions: append([]missioncontrol.CampaignZohoEmailOutboundAction(nil), actions...),
		CreatedAt:                        now,
		UpdatedAt:                        now,
		StartedAt:                        now,
		ActiveStepAt:                     now,
	}
	normalizedReplies := make([]missioncontrol.FrankZohoInboundReply, 0, len(replies))
	for _, reply := range replies {
		nextRuntime, changed, err := missioncontrol.AppendFrankZohoInboundReply(runtime, reply)
		if err != nil {
			t.Fatalf("AppendFrankZohoInboundReply(%q) error = %v", jobID, err)
		}
		if !changed {
			t.Fatalf("AppendFrankZohoInboundReply(%q) changed = false, want reply append", jobID)
		}
		runtime = nextRuntime
