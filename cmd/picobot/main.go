...
func installMissionRuntimeChangeHookWithExtension(cmd *cobra.Command, ag *agent.AgentLoop, after func()) {
	...
	if storeRoot != "" {
		...
		ag.SetMissionRuntimeProjectionHook(func(job *missioncontrol.Job, runtime missioncontrol.JobRuntimeState, _ *missioncontrol.RuntimeControlContext) error {
			...
			return writeProjectedMissionStatusSnapshotWithModelStatus(statusFile, missionStatusSnapshotMissionFile(cmd), storeRoot, missionRequired, jobID, time.Now(), missionStatusModelRouteStatus(ag), missionStatusModelControlMetricsStatus(ag), missionStatusModelHealthStatus(ag))
		})
